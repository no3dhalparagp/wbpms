...
function MenuItem({ item, userRole }: { item: MenuItemProps; userRole: Role }) {
  ...
  const normalizedRole = userRole === "superadmin" ? "SUPER_ADMIN" : (userRole.toUpperCase() as "ADMIN" | "STAFF")
      ...
      {item.submenu && (
        ...
        >
          {item.subMenuItems.map((subItem) => (
            <MenuItem key={subItem.menuItemText} item={subItem} userRole={userRole} />
          ))}
        </div>
      )}
    </div>
