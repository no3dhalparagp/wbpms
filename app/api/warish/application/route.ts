  ...
  try {
    ...
    if (type !== "acknowlegment" && type !== "certificate") {
      ...
      return NextResponse.json(
        {
          error:
            "Invalid search type. Must be either 'acknowlegment' or 'certificate'",
        },
        { status: 400 }
      );
    }
    ...
    const allApplications = await prisma.warishApplication.findMany({
      select: {
        acknowlegment: true,
      },
      take: 5,
    ...
    let application = await prisma.warishApplication.findFirst({
      where: {
        ...(type === "acknowlegment"
    ...
    if (!application) {
      application = await prisma.warishApplication.findFirst({
        where: {
          ...(type === "acknowlegment"
    ...
    if (!application) {
      ...
      const similarApplications = await prisma.warishApplication.findMany({
        where: {
          acknowlegment: {
            startsWith: "ACK/LH/",
          },
