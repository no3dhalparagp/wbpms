...
export const addPaymentDetails = async (
  ...
  try {
    ...
    const existingWork = await prisma.worksDetail.findUnique({
    ...
    const [incomeTax, labourWelfareCess, tdsCgst, tdsSgst, securityDeposit] =
      await Promise.all([
        prisma.incomeTaxRegister.create({
          data: {
        ...
        prisma.labourWelfareCess.create({
          data: {
        ...
        prisma.secrutityDeposit.create({
          data: {
