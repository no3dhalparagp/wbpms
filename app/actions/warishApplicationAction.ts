...
async function generateAcknowledgmentNumber() {
  ...
  if (!user?.gramPanchayatId) {
    throw new Error("User is not associated with a Gram Panchayat");
  }

  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();

  // Get the latest application for the current year scoped to the user's GP
  ...
  if (latestApplication?.acknowlegment) {
