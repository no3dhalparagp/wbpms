...
import { currentgpuserlogin, currentUser } from "@/lib/auth";

type VendorSchemaType = z.infer<typeof vendorSchema>;
export async function fetchallApproveActionPlanDetails() {
  try {
    const schme = await prisma.approvedActionPlanDetails.findMany({
      where: {
        isPublish: false,
      },
    });
...
export async function createschme(values: z.infer<typeof actionplanschema>) {
  ...
  try {
    const scheme = await prisma.approvedActionPlanDetails.create({
      data: {
...
export async function vendorSchemaAction(values: VendorSchemaType) {
  ...
  try {
    ...
    const existingVendor = await prisma.agencyDetails.findFirst({
      where: { name },
    });
    ...
    const newVendor = await prisma.agencyDetails.create({
      data: {
        name,
        email,
        mobileNumber,
