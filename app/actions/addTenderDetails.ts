...
export const addTenderDetails = async (
  ...
  try {
    const nitDetails = await prisma.nitDetails.findUnique({
      where: {
        id: tenderId,
      },
    });
    ...
    const slno = await prisma.worksDetail.count({
      where: {
    ...
    const tender = await prisma.worksDetail.create({
      data: {
        workslno: slnon,
        nitDetailsId: nitDetails.id,
